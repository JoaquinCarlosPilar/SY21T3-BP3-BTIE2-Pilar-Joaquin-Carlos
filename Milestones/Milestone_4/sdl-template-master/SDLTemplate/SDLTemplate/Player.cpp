...
{ 
	...
	// This delete bullets that are off the game screen (AltBullet)
	for (int i = 0; i < altbullets.size(); i++)
	{
		if (altbullets[i]->getPositionX() > SCREEN_WIDTH)
		{
			// This caches the variable so we can delete it later
	...
	// This delete bullets that are off the game screen (AltBullet2)
	for (int i = 0; i < altbullets2.size(); i++)
	{
		if (altbullets2[i]->getPositionX() > SCREEN_WIDTH)
		{
			// This caches the variable so we can delete it later
			AltBullet2* altbullet2ToErase = altbullets2[i];
			altbullets2.erase(altbullets2.begin() + i);
			delete altbullet2ToErase;
	...
	// This is the button to shoot
	// F keybind
	if (app.keyboard[SDL_SCANCODE_F] && currentReloadTime == 0)
	{
		SoundManager::playSound(sound);
		Bullet* bullet = new Bullet(x - 10 + width, y - 5 + height / 2, 1, 0, 10, Side::PLAYER_SIDE);
	...
	{
		SoundManager::playSound(sound);
		AltBullet* altbullet = new AltBullet(x - 60 + width, y - 31 + height / 2, 1, 0, 10, Side::PLAYER_SIDE);
	...
	{
		SoundManager::playSound(sound);
		AltBullet2* altbullet2 = new AltBullet2(x - 60 + width, y + 23 + height / 2, 1, 0, 10, Side::PLAYER_SIDE);
